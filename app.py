...
def get_version():
    try:
        with open('./version.properties') as pf:
            return f'Version {pf.read()}'
...
def upload():
    return render_template('upload.html', app_version=get_version(), title=TITLE)
...
def process_files():
    try:
        ...
        if filenames:
            ...
            return render_template('downloads.html', filename=report.replace('generated/', ''), app_version=get_version(), title=TITLE)
