    ...
    if 'Product' in event_dict:
        if isinstance(event_dict['Product'], list):
            ...
            for product in event_dict['Product']:
                ...
                _ = record.copy()           # copy original record
                _.update(missing_columns)   # update the duplicate record with missing fields

                # Add CheckReceivedTimestamp and TimeDelta
                _['CheckReceivedTimestamp'], _['TimeDelta'] = dt_string, time_delta
                product_records.append(_)
...
def get_catalina_pos_api_events(log_file):
    ...
    for index, line in enumerate(log_file_content):
        ...
        if line.startswith('INFO'): # Check for product event only if line begins with "INFO" - optimization
            if '<ProductEvent ' in line:
        ...
        # Parse XML records spanning multiple lines
        # Start multiline record
        if line.startswith('<ProductEvent '):
            time_stamp = log_file_content[index - 2] if 'AM' in log_file_content[index - 2] or 'PM' in log_file_content[index - 2] else log_file_content[index - 3]
        ...
        if line.startswith('</ProductEvent>'):
            ...
            try:
                ...
                log.debug(f'XML Parser Error: {e} in record: \n{new_record}')
                continue
            ...
            if isinstance(r, list):
                for n_record in r:
                    if n_record not in records:         # exclude duplicates
                        records.append(n_record)
            else:
                if r not in records:                    # exclude duplicates
                    records.append(r)
    ...
    with pd.ExcelWriter(path=excel_out, engine='xlsxwriter', datetime_format='mmm d yyy hh:mm:ss') as writer:
        for log_file in log_files:
            try:
                ...
                log.exception(f'failed to process {log_file}: {e}')
